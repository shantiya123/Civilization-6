package Game.Client.Views.TradingPostPanel;
...
/**
 * Trading Post interaction panel, shown when the player clicks a Trading
 * Post hex they control. Reads only from TradingPostPanelState; every
 * button click delegates to a named intent method on the state
 * (Panel -> State -> Controller, per the panel design contract). The
 * state's intent methods are currently stubs, so this panel is
 * display/interaction-only for now.
 */
public final class TradingPostPanel extends JPanel {
    public static final int PANEL_WIDTH = 260;
    public static final int PANEL_HEIGHT = 300;

    private static final Color PANEL_BACKGROUND = new Color(24, 26, 27, 235);
    private static final Color COPPER = new Color(184, 115, 51);
    private static final Color TEXT = new Color(224, 214, 201);

    private final TradingPostPanelState state;

    private final JLabel headerTitleLabel = titleLabel("Trading Post");
    private final JLabel rateLabel = captionLabel("Conversion Rate: 80%");
    private final JLabel noTradingPostLabel = captionLabel("No Trading Post selected.");
    ...
    public TradingPostPanel(TradingPostPanelState state) {
        this.state = state;
        ...
        setBorder(BorderFactory.createCompoundBorder(
                BorderFactory.createLineBorder(COPPER, 2), BorderFactory.createEmptyBorder(12, 14, 14, 14)));
    ...
    private JPanel createBody() {
        ...
        body.add(centeredLabel(noTradingPostLabel));
    ...
    public void refresh() {
        ...
        boolean canTrade = !tradeableTypes.isEmpty();
        ...
        noTradingPostLabel.setVisible(!canTrade);
